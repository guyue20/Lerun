...
//    @Select("select distinct  t.question_id,\n" +
//            "        t.question_main,\n" +
//            "        t.achievement,\n" +
//            "\t\ttd.descript_name,\n" +
//            "        tc.anwers_main,\n" +
...
//            "left join GT_Exam_Paper tp on tr.paper_id=tp.paper_id where t.question_id in(${wrdss})")
    @Select("select distinct t.question_id, t.question_main, t.achievement, td.descript_name, tc.anwers_main,\n" +
            " ta.anwers_id, ta.question_remark as questionremark, tcon.name,tp.paper_name \n" +
            "FROM GT_Exam_Questionsbank t \n" +
            "left join GT_Exam_Descript td on t.describe_id=td.descript_id \n" +
            "left join GT_Exam_Choice tc on t.question_id=tc.question_id \n" +
            "left join GT_Exam_AI ta on t.question_id=ta.question_id \n" +
            "left join GT_Exam_Course tcon on tcon.subject_id=t.subject_id \n" +
            "left join GT_Exam_PaQu tpq on t.question_id=tpq.question_id\n" +
            "left join GT_Exam_Paper tp on tpq.paper_id=tp.paper_id\n" +
            "where t.question_id in(${wrdss})")
    public List<GTExamRecordwors> findworall(@Param("wrdss") String wrdss);
}